...
{
	sscanf(inputString, "%s", frontString);
...
void COBJLoader::ReadTriangleFaceVertTexNorm(char* line, int matId)
{
	...
	for (int x = 0; x < 3; x++)
	{
		...
		sscanf(currentArg, "%i/%i/%i", &m_uiVertIdx[x], &m_uiTexCoordIdx[x], &m_uiNormalIdx[x]);
		memcpy(line, remStr, 255);
	}
...
bool COBJLoader::LoadMTLFile(const char* mainName, const char* filename)
{
...

	{
		...
		else if (identifierStr == "newmtl")
		{
			...
			sscanf(line, "%s", newMaterial.m_carrMatName);
...

		{
			...
			sscanf(line, " %f", &newMaterial.m_fTransparency);
...

		{
			...
			sscanf(line, "llum %i", &newMaterial.m_iIlluminationModel);
...

		{
			...
			sscanf(line, "%f %f %f", &newMaterial.m_farrAmbientCol[0], &newMaterial.m_farrAmbientCol[1], &newMaterial.m_farrAmbientCol[2]);
...

		{
			...
			sscanf(line, "Kd %f %f %f", &newMaterial.m_farrDiffuseCol[0], &newMaterial.m_farrDiffuseCol[1], &newMaterial.m_farrDiffuseCol[2]);
...

		{
			...
			sscanf(line, "Ks %f %f %f", &newMaterial.m_farrSpecularCol[0], &newMaterial.m_farrSpecularCol[1], &newMaterial.m_farrSpecularCol[2]);
...

		{
			...
			sscanf(line, "%s", &newMaterial.m_carrTextureName);
	...
	while (!feof(theFile))
	{
		...
		case 'v':   //It's a vertex or vertex attribut
		{
			...
			case '\t':
			{
				...
				sscanf(line, " %f %f %f", &thePoints[0], &thePoints[1], &thePoints[2]); //get the vertex coords
			...
			case 'n':
			{
				...
				sscanf(line, " %f %f %f", &theNormals[0], &theNormals[1], &theNormals[2]); //get the normal coords
			...
			case 't':
			{
				...
				sscanf(line, " %f %f", &theTex[0], &theTex[1]); //get the texture coords
				//theTex[1] = 1-theTex[1];							
